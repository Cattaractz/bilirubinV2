...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAgeDisplay() {
        ...
        const diffHours = (now - birthDate) / (1000 * 60 * 60);

        if (diffHours < -0.01) {
            display.innerText = "Fremtid?";
            display.style.color = "#d9534f";
        } else {
            const val = isTimeComplete ? (Math.round(diffHours * 10) / 10) : Math.floor(diffHours);
            display.innerText = (isTimeComplete ? "" : "Minst ") + val + " timer";
            display.style.color = isTimeComplete ? "#5abcb6" : "#4cb98f";
        }
    }
...
function calculateBilirubin() {
    ...
    if (isOverExchange) {
        alertHtml = `<div style="margin-top: 15px; padding: 15px; background-color: #f8d7da; border-left: 6px solid #dc3545; color: #721c24; border-radius: 8px;">
                ...
                <span style="font-size: 0.95rem; line-height: 1.4;">
                    Verdien på <strong>${actV} µmol/L</strong> er over grensen for utskiftningstransfusjon (~${Math.floor(exchangeLimit)} µmol/L). 
    ...
    if (!isNaN(t2) && !isNaN(v2)) {
        ...
        if (isOverExchange){
            dynamiskTekst = `Pasientens verdi på ${v2} er ~ ${diffOverExchange} µmol/L over utskiftningsgrensen. `;
        }

        else if (isOverLimit) {
            dynamiskTekst = `Verdien på ${v2} er ${avstand} µmol/L over behandlingsgrense på ~ ${Math.floor(currentLimit)}`;
        }
        else if (v1 === v2) {
        ...
        let krysningTekst = "";
        if (slope > 0 && intersectX) {
            const timerFraSiste = Math.round(intersectX - t2);
            krysningTekst = ` og vil krysse grensen ved ca. ${Math.floor(intersectX)} timer alder (~${timerFraSiste}t fra siste prøve)`;
        } else if (slope <= 0) {
            if (slope < 0){
                krysningTekst = ". Verdien er synkende";
            }
            else {
                krysningTekst = ". Verdien er stabil, det kan være behov for en kontroll.";
            }
        ...
        if (isOverExchange){
            dynamiskTekst = `Pasientens verdi på ${v1} er ~ ${diffOverExchange} µmol/L over utskiftningsgrensen. `;
        } else {
           dynamiskTekst = `Pasienten har en verdi på ${actV} µmol/L, som er ${avstand} µmol/L ${overUnder} behandlingsgrensen på ~ ${Math.floor(currentLimit)}`; 
        }
...
function parseSmartDate(inputStr) {
    ...
    if (parts.length >= 2) {
        ...
        // If year is missing or shorthand (like '26'), assume current year
        if (!parts[2]) {
            year = currentYear;
        } else {
        ...
        // Validation: Check if the date is actually valid (e.g., not 31/02)
...
function openInfoModal() {
    ...
    if (data) {
        infoParagraph.innerHTML = `<strong>${data.title}</strong><br><br>${data.text.replace(/\n/g, '<br>')}`;
